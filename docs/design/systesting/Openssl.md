...
> **Production status:** `bnd-openssl` is generated by the direct
> `bnd-clang` → RDL/WinMD → `bnd-bindgen` pipeline. Sections explicitly
> labeled historical describe the earlier `bnd-winmd` implementation and
> fixture work; they are not current production instructions.
...
| `openssl/tls1.h` | `ssl` | TLS protocol declarations and defining-header ownership |
...
The generator defaults native functions to `crypto` and assigns definitions
owned by `ssl.h` or `tls1.h` to `ssl`:

```text
all active headers -> RDL by defining header
                   -> flat temporary WinMD
                   -> defining-header remap + reference repair
                   -> bnd-openssl.winmd (`openssl.<header>` namespaces)
...
`bnd-openssl-gen` runs the direct-Clang pipeline offline and produces the
checked-in `bnd-openssl` source tree.

```
bnd-openssl-gen/                    ← generator (cargo run -p bnd-openssl-gen)
├── Cargo.toml
├── src/
│   ├── clang.rs                    ← Clang, RDL, canonical remap, external reference, bindgen
│   ├── lib.rs                      ← exports the production generator
│   └── main.rs                     ← fn main() { generate(&bnd_openssl_dir) }
└── tests/
    └── clang_up_to_date.rs         ← complete freshness + idempotence check

bnd-openssl/                        ← product crate
├── Cargo.toml                      ← package metadata + generated features
├── winmd/
│   └── bnd-openssl.winmd           ← canonical defining-header metadata
├── src/
│   ├── lib.rs                      ← hand-written: pub mod openssl;
│   └── openssl/                    ← GENERATED by bnd-bindgen package mode
...
This regenerates Linux first, then OpenSSL. The production metadata path is:

```text
one GNU C11 OpenSSL translation unit
              |
              | bnd-linux/winmd/bnd-linux.winmd reference
              v
 ...
 flat temporary OpenSSL WinMD
              |
              | defining-header remap + external-scope RDL repair
              v
 bnd-openssl/winmd/bnd-openssl.winmd
       (canonical `openssl.<header-module>` namespaces)
              |
              | one full `libc` external Rust reference
              v
 bnd-bindgen
              |
              v
...
WinMD has `openssl.<defining-header-module>` namespaces matching the 18
generated Rust modules:
`asn1`, `bio`, `bn`, `buffer`, `comp`, `conf`, `conftypes`, `core`, `crypto`,
`err`, `evp`, `rand`, `rsa`, `sha`, `ssl`, `tls1`, `types`, and `x509`.

Passing both WinMD files through `--in` resolves metadata references, while
the generic `bnd-bindgen` namespace-preserving reference maps the complete
`libc` namespace to `bnd_linux`. Because metadata and Rust namespaces agree,
no per-type OpenSSL/Linux route table is needed.
...
generated routing. The production crate checks in its Rust modules, canonical
...
## Historical `bnd-winmd` Configuration

The TOML partition configuration below records the earlier implementation.
The production direct-Clang generator now defines its root headers, library
policy, and external routes in `bnd-openssl-gen/src/clang.rs`.
...
## Historical API Surface Expectations
...
## Historical Extraction Challenges
...
## Historical Partition Ordering Strategy
...
## Historical `bnd-winmd` Generator Design

Before the direct-Clang cutover, `generate(output_dir: &Path)` ran
...
That product crate depended on the former nested `bnd-linux` features:
...
At that time the product crate needed a `build.rs` to emit
...
## Historical Roundtrip Tests — `bnd-winmd/tests/roundtrip_openssl.rs`
...
| **Cross-WinMD type references** | POSIX types (`tm`, `FILE`, `pthread_once_t`, `off_t`, etc.) remain external TypeRefs to the canonical bnd-linux WinMD |
| **WinMD format** | `bnd-bindgen` accepts the canonical OpenSSL and Linux metadata together |
...
In the earlier pipeline, bnd-winmd extracted `lhash_st_ERR_STRING_DATA` with the union as a field
...
## Historical Comparison with Existing System Tests
