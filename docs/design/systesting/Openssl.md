...
all active headers -> RDL by defining header
                   -> bnd-openssl.winmd (flat `openssl` namespace)
...
├── Cargo.toml
├── src/
│   ├── clang.rs                    ← Clang, RDL, metadata remap, external routes, bindgen
...
├── winmd/
│   └── bnd-openssl.winmd           ← canonical flat `openssl` metadata
├── src/
...
one GNU C11 OpenSSL translation unit
 ...
 bnd-openssl/winmd/bnd-openssl.winmd
       (flat canonical `openssl` namespace)
              |
              | RDL-derived defining-header ownership
              v
 temporary package-remapped WinMD
              |
              | exact external Rust routes
              v
 bnd-bindgen
              |
              v
...
`bnd_linux::libc::<defining-header-module>`, excludes local copies and feature
gates, and keeps the concrete OpenSSL/Linux routes in `bnd-openssl-gen`.
