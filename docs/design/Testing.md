...
Prove the full pipeline works: **C header → bnd-winmd (winmd) → windows-bindgen
...
└── multi/                   ← multi-partition  (e2e-multi)
    ...
    └── multi.toml
        │
        ├── bnd-winmd  ──→ .winmd                  (winmd metadata)
        │                        │
...
Workspace members: `["bnd-winmd", "tests/simple-impl", "tests/e2e-simple", "tests/e2e-multi"]`
...
- **Extend**: unions, bitfields, nested structs as bnd-winmd supports them
...
Validate that bnd-winmd correctly handles multiple partitions (namespaces)
...
3. **Add `tests/e2e-multi/` crate** — ✅ done
   Separate crate with `build.rs` running `multi.toml` through bnd-winmd →
...
| `bnd-linux/` (package mode, POSIX + Linux syscalls) | ✅ |
| `bnd-linux-gen/` (generator, `--package` mode) | ✅ |
