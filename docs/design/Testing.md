...
│   ├── Cargo.toml
│   └── src/lib.rs
├── e2e-test/             ← single-partition E2E (simple.h → SimpleTest)
│   ├── Cargo.toml
│   ├── build.rs
│   └── src/
...
Workspace members: `["bindscrape", "tests/simple-impl", "tests/e2e-test", "tests/e2e-multi"]`

Run:
- Single-partition: `cargo test -p e2e-test`
...
- **`--flat` for single-partition, namespaced for multi**: `e2e-test` uses
...
| C construct | `--flat` (e2e-test) | Namespaced (e2e-multi) |
|---|---|---|
